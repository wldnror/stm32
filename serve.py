...
# 시스템 업데이트 메뉴는 제거하고 openocd 관련 명령어만 사용합니다.
commands = [
...
def git_pull():
    ...
    with open(shell_script_path, 'w') as script_file:
        ...
        # 원격의 모든 브랜치를, -> 표시가 없는 각 줄을 while-read 루프로 처리하여 로컬에 강제로 생성/덮어쓰기
        script_file.write("git branch -r | grep -v '\\->' | while read remote; do\n")
