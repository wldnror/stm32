...
from app_config import (
    OPENOCD_IFACE, OPENOCD_TARGET,
    STM32_RECENT_UNLOCK_TTL_SEC, STM32_FALLBACK_PROBE_TIMEOUT_SEC,
    STM32_DETECT_CACHE_TTL_SEC, STM32_DETECT_TIMEOUT_SEC,
    STM32_UNLOCK_TIMEOUT_SEC, STM32_LOCK_TIMEOUT_SEC,
    FLASH_KB_THRESHOLD, GENERAL_ROOT, TFTP_ROOT,
    GENERAL_DIRNAME, TFTP_DIRNAME,
)
...
def _detect_flash_kb_by_probe(timeout=STM32_FALLBACK_PROBE_TIMEOUT_SEC) -> Optional[int]:
    ...
    txt = (out or "") + "\n" + (err or "")
    return _parse_openocd_flash_kb(txt)
...
def check_stm32_connection():
    if st.is_command_executing:
        return False
    try:
        rc, _, _ = run_openocd_capture(["init", "exit"], timeout=0.55)
        ok = (rc == 0)
        with st.stm32_state_lock:
            st.connection_failed_since_last_success = not ok
            st.connection_success = ok
        return ok
    except Exception:
        with st.stm32_state_lock:
